...
支持**多账号**、**深色模式**、**继续上次播放**、图文/图集与**实况（动图）**、批量取消点赞、`.dylist` 备份迁移。
...
> 主界面（浅色）：顶栏是搜索与「🌙 深色 / 使用教程 / 常见问题」；左栏是分类与手动刷新，**左下角是账号入口**（点开可切换 / 新增 / 改名 / 退出登录）；仪表盘显示已采集条数、视频 / 图集 / 占用（MB）/ 年份，右侧红色按钮即「采集」；下方是年份月份筛选、洗牌播放、自动连播与卡片网格。
...
- **一账号一环境**：每个账号有独立的 WebView2 登录态、独立的喜欢列表与采集进度，互不干扰
- **面板**（左下角账号区）：切换、添加、改名、退出登录、删除；切换约几秒（重建浏览器环境），期间有居中转圈提示
- **同一抖音号只算一个账号**：在新账号里登录了已存在的抖音号时，两份数据自动合并（旧目录改名留档），不会出现两份互相矛盾的本地数据
- **删除即留档**：删除账号只是把数据目录与登录态改名归档（形如 `user2_deleted_MMdd_HHmmss`），不做物理删除
- **退出登录**只清本机登录信息，采集数据保留
...
- 新增**多账号**：独立数据域与登录态、面板切换 / 新增 / 改名 / 删除，同一抖音号自动归一
- 新增**深色模式**：应用级设置，切换账号保持
- 新增**继续上次播放**：记住条目与播放位置，主界面一键回到上次进度
- 新增**实况（动图）图集**支持，播放节奏对齐抖音
- 新增播放页顶部**拖动窗口 / 最小化**，主界面账号入口替代原登录按钮
- 修复：**接口报错被误判为「采集完成」**（采纳社区 PR #6 的响应校验加固）
- 修复：播放页在窗口拖动 / 缩放后偶发「内容偏到左上角」
- 优化：点播放**先拉起播放页再取链**，首帧等待明显缩短；直链探测改为「首活即用」
- 优化：启动时不再复制历史数据目录（零拷贝引用，秒开）
