...
- **视频有声音但没画面 / 整个播放页白屏？** 两种情形：① 部分内容为 H.265 编码、系统缺 HEVC 解码 → 会自动尝试备用链接，仍不行请给 Win10 装「HEVC 视频扩展」；② 连播放器按钮都看不到的整页白、而用 Edge 打开抖音正常 → 多为 WebView2 Runtime 版本太旧（与 Edge 独立更新），安装最新 Evergreen 版后重启应用。
- **「删除」和「取消点赞」的区别？** 「删除」只把该条移出本地列表（抖音点赞仍在）；「取消点赞」才是真正去抖音取消点赞——只有确认成功才移除本地，批量逐条节流、可随时停止，触发风控会自动暂停并弹验证，未成功的保留本地。
- **窗口边缘拖不动、不能自由缩放？** 应用是无边框窗口，边缘缩放区域被播放内核占满，属架构限制；调整大小请用右上角「最大化 / 还原」，或 `Win`+`↑` / `Win`+`↓`（最大化 / 还原）、`Win`+`←→`（贴半屏）。
- **播放列表怎么看不到全部几千条？** 侧栏围绕当前播放条目按需加载、向下滚动自动续载（避免一次渲染几万行）；该队列仅存在于本次播放会话，关闭应用不保存。
